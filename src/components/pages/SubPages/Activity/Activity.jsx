...
export default function Activity() {
    ...
    const [allActivitys, setAllActivitys] = useState([])
    const [currentPage, setCurrentPage] = useState(1);
    const [sortField, setSortField] = useState("Name");
    const [sortOrder, setSortOrder] = useState("asc");
    const rowsPerPage = 10;

    const sortActivitys = (activitys, field, order) => {
        return activitys.sort((a, b) => {
    ...
    const sortedActivitys = sortActivitys([...allActivitys], sortField, sortOrder);
    ...
    const fetchAllActivity = () => {
        getAllActivity(userData.UserId, userData.PCSGroupId, userData.PCSId, userData.ClientId, setAllActivitys)
    }
    ...
    const currentActivitys = sortedActivitys.slice(indexOfFirstRow, indexOfLastRow);

    const totalPages = Math.ceil(allActivitys.length / rowsPerPage);
                ...
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuCheckboxItem checked={sortField === "ActivityTypeDesc"} onClick={() => handleSortChange("ActivityTypeDesc")}> Type </DropdownMenuCheckboxItem>
                    ...
                    <Button size="sm" className="h-8 gap-1 bg-sky-600">
                        <PlusCircle className="h-3.5 w-3.5" />
                        <span className="sr-only  sm:not-sr-only sm:whitespace-nowrap">Add Activities</span>
                    </Button>
                </div>
            </div>
            <Card className="w-full overflow-y-auto">
                <CardHeader>
                    <CardTitle className='text-sky-600'>Activities</CardTitle>
                ...
                <CardContent>
                    <div className="w-full overflow-x-hidden">
                        <Table className="min-w-full">
                            <TableHeader>
                                <TableRow>
                                    <TableHead className="hidden w-[100px] sm:table-cell"> <span className="sr-only">img</span></TableHead>
                                ...
                                {currentActivitys.length > 0 ? (
                                    currentActivitys.map((activity) => (
                                        <TableRow key={activity.activity_id} >
                                            <TableCell className="hidden sm:table-cell"> <span>📝</span></TableCell>
